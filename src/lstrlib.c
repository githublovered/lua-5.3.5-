...
** Standard library for string operations and pattern-matching    字符串操作和匹配标准库，Lua里面的模式匹配及其定义的体系
...
** an unsigned char.
模式匹配期间模式可以执行的最大捕获数。此限制是任意的，但必须适合无符号字符。
*/
...
/* macro to 'unsign' a character 宏“unsign”字符 */
#define uchar(c)	((unsigned char)(c))

6222 620810016942472
/*
** Some sizes are better limited to fit in 'int', but must also fit in
** 'size_t'. (We assume that 'lua_Integer' cannot be smaller than 'int'.)
有些尺寸最好限制在"int"中，但也必须适合"size_t"。（我们假设“lua_integer”不能小于“int”。）
*/
...
/* translate a relative string position: negative means back from end 翻译一个相对的字符串位置：负表示从末尾向后*/
